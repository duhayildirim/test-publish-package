...
export interface ClientOptions {
  /**
   * Defaults to process.env['hyperbee_API_KEY'].
   */
  apiKey?: string | undefined;

  /**
   * Defaults to process.env['hyperbee_ORG_ID'].
   */
  organization?: string | null | undefined;

  /**
   * Defaults to process.env['hyperbee_PROJECT_ID'].
   */
   ...
   *
   * Defaults to process.env['hyperbee_BASE_URL'].
   */
...
/**
 * API Client for interfacing with the hyperbee API.
 */
export class hyperbee extends Core.APIClient {
  apiKey: string;
   ...
   * API Client for interfacing with the hyperbee API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['hyperbee_API_KEY'] ?? undefined]
   * @param {string | null | undefined} [opts.organization=process.env['hyperbee_ORG_ID'] ?? null]
   * @param {string | null | undefined} [opts.project=process.env['hyperbee_PROJECT_ID'] ?? null]
   * @param {string} [opts.baseURL=process.env['hyperbee_BASE_URL'] ?? https://api.hyperbee.com/v1] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = Core.readEnv('hyperbee_BASE_URL'),
    apiKey = Core.readEnv('hyperbee_API_KEY'),
    organization = Core.readEnv('hyperbee_ORG_ID') ?? null,
    project = Core.readEnv('hyperbee_PROJECT_ID') ?? null,
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.hyperbeeError(
        "The hyperbee_API_KEY environment variable is missing or empty; either provide it, or instantiate the hyperbee client with an apiKey option, like new hyperbee({ apiKey: 'My API Key' }).",
      );
    }
    ...
    const options: ClientOptions = {
      ...
      project,
      ...opts,
      baseURL: baseURL || `https://api.hyperbee.com/v1`,
    };

    if (!options.dangerouslyAllowBrowser && Core.isRunningInBrowser()) {
      throw new Errors.hyperbeeError(
        "It looks like you're running in a browser-like environment.\n\nThis is disabled by default, as it risks exposing your secret API credentials to attackers.\nIf you understand the risks and have appropriate mitigations in place,\nyou can set the `dangerouslyAllowBrowser` option to `true`, e.g.,\n\nnew hyperbee({ apiKey, dangerouslyAllowBrowser: true });\n\nhttps://help.hyperbee.com/en/articles/5112595-best-practices-for-api-key-safety\n",
      );
    }
  ...
  protected override defaultHeaders(opts: Core.FinalRequestOptions): Core.Headers {
    return {
      ...
      'hyperbee-Organization': this.organization,
      'hyperbee-Project': this.project,
  ...
  static hyperbee = this;
  static DEFAULT_TIMEOUT = 600000; // 10 minutes

  static hyperbeeError = Errors.hyperbeeError;
...
/** API Client for interfacing with the Azure hyperbee API. */
export interface AzureClientOptions extends ClientOptions {
  /**
   * Defaults to process.env['hyperbee_API_VERSION'].
   */
  apiVersion?: string | undefined;

  /**
   * Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.hyperbee.com/`
   */
  ...
  /**
   * Defaults to process.env['AZURE_hyperbee_API_KEY'].
   */
  apiKey?: string | undefined;
...
/** API Client for interfacing with the Azure hyperbee API. */
export class Azurehyperbee extends hyperbee {
   ...
   * API Client for interfacing with the Azure hyperbee API.
   *
   * @param {string | undefined} [opts.apiVersion=process.env['hyperbee_API_VERSION'] ?? undefined]
   * @param {string | undefined} [opts.endpoint=process.env['AZURE_hyperbee_ENDPOINT'] ?? undefined] - Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.hyperbee.com/`
   * @param {string | undefined} [opts.apiKey=process.env['AZURE_hyperbee_API_KEY'] ?? undefined]
   * @param {string | undefined} opts.deployment - A model deployment, if given, sets the base client URL to include `/deployments/{deployment}`.
   * @param {string | null | undefined} [opts.organization=process.env['hyperbee_ORG_ID'] ?? null]
   * @param {string} [opts.baseURL=process.env['hyperbee_BASE_URL']] - Sets the base URL for the API.
   * @param {number} [opts.timeout=10 minutes] - The maximum amount of time (in milliseconds) the client will wait for a response before timing out.
   * @param {number} [opts.httpAgent] - An HTTP agent used to manage HTTP(s) connections.
   * @param {Core.Fetch} [opts.fetch] - Specify a custom `fetch` function implementation.
   * @param {number} [opts.maxRetries=2] - The maximum number of times the client will retry a request.
  ...
  constructor({
    baseURL = Core.readEnv('hyperbee_BASE_URL'),
    apiKey = Core.readEnv('AZURE_hyperbee_API_KEY'),
    apiVersion = Core.readEnv('hyperbee_API_VERSION'),
    endpoint,
    deployment,
    ...
    if (!apiVersion) {
      throw new Errors.hyperbeeError(
        "The hyperbee_API_VERSION environment variable is missing or empty; either provide it, or instantiate the Azurehyperbee client with an apiVersion option, like new Azurehyperbee({ apiVersion: 'My API Version' }).",
      );
    }
    ...
    if (!azureADTokenProvider && !apiKey) {
      throw new Errors.hyperbeeError(
        'Missing credentials. Please pass one of `apiKey` and `azureADTokenProvider`, or set the `AZURE_hyperbee_API_KEY` environment variable.',
      );
    }
    ...
    if (!baseURL) {
      if (!endpoint) {
        endpoint = process.env['AZURE_hyperbee_ENDPOINT'];
      }

      if (!endpoint) {
        throw new Errors.hyperbeeError(
          'Must provide one of the `baseURL` or `endpoint` arguments, or the `AZURE_hyperbee_ENDPOINT` environment variable',
        );
      }

      baseURL = `${endpoint}/hyperbee`;
    } else {
      if (endpoint) {
        throw new Errors.hyperbeeError('baseURL and endpoint are mutually exclusive');
      }
    }
  ...
  protected override async prepareOptions(opts: Core.FinalRequestOptions<unknown>): Promise<void> {
    ...
    if (token) {
      ...
      throw new Errors.hyperbeeError('Unable to handle auth');
    }
    return super.prepareOptions(opts);
  }
}
