...
import { type CompletionUsage } from 'hyperbee/resources/completions';
import {
  type ChatCompletion,
...
import { hyperbeeError } from 'hyperbee/error';
import {
...
import hyperbee from '../index';
import { isAutoParsableTool, parseChatCompletion } from 'hyperbee/lib/parser';
...
> extends EventStream<EventTypes> {
  ...
  async finalChatCompletion(): Promise<ParsedChatCompletion<ParsedT>> {
    ...
    if (!completion) throw new hyperbeeError('stream ended without producing a ChatCompletion');
    return completion;
  }
  ...
  #getFinalMessage(): ChatCompletionMessage {
    ...
    throw new hyperbeeError('stream ended without producing a ChatCompletionMessage with role=assistant');
  }
  ...
  protected async _runFunctions<FunctionsArgs extends BaseFunctionsArgs>(
    ...
    for (let i = 0; i < maxChatCompletions; ++i) {
      ...
      const message = chatCompletion.choices[0]?.message;
      if (!message) {
        throw new hyperbeeError(`missing message in ChatCompletion response`);
      }
  ...
  protected async _runTools<FunctionsArgs extends BaseFunctionsArgs>(
    ...
    const inputTools = params.tools.map((tool): RunnableToolFunction<any> => {
      if (isAutoParsableTool(tool)) {
        if (!tool.$callback) {
          throw new hyperbeeError('Tool given to `.runTools()` that does not have an associated function');
        }
