import * as Core from 'hyperbee/core';
import {
  hyperbeeError,
  APIUserAbortError,
...
} from 'hyperbee/resources/chat/completions';
import {
  AbstractChatCompletionRunner,
  type AbstractChatCompletionRunnerEvents,
} from './AbstractChatCompletionRunner';
import { type ReadableStream } from 'hyperbee/_shims/index';
import { Stream } from 'hyperbee/streaming';
import hyperbee from 'hyperbee/index';
import { ParsedChatCompletion } from 'hyperbee/resources/beta/chat/completions';
import {
  ...
  implements AsyncIterable<ChatCompletionChunk>
{
  ...
  #emitContentDoneEvents(choiceSnapshot: ChatCompletionSnapshot.Choice) {
    ...
    if (this.ended) {
      throw new hyperbeeError(`stream has ended, this shouldn't happen`);
    }
    const snapshot = this.#currentChatCompletionSnapshot;
    if (!snapshot) {
      throw new hyperbeeError(`request ended without sending any chunks`);
    }
    this.#currentChatCompletionSnapshot = undefined;
    this.#choiceEventStates = [];
  ...
  const completion: ChatCompletion = {
    ...
    choices: choices.map(
      ({ message, finish_reason, index, logprobs, ...choiceRest }): ChatCompletion.Choice => {
        if (!finish_reason) {
          throw new hyperbeeError(`missing finish_reason for choice ${index}`);
        }

        const { content = null, function_call, tool_calls, ...messageRest } = message;
        const role = message.role as 'assistant'; // this is what we expect; in theory it could be different which would make our types a slight lie but would be fine.
        if (!role) {
          throw new hyperbeeError(`missing role for choice ${index}`);
        }

        if (function_call) {
          const { arguments: args, name } = function_call;
          if (args == null) {
            throw new hyperbeeError(`missing function_call.arguments for choice ${index}`);
          }

          if (!name) {
            throw new hyperbeeError(`missing function_call.name for choice ${index}`);
          }
          ...
          return {
            ...
            message: {
              ...
              tool_calls: tool_calls.map((tool_call, i) => {
                ...
                if (id == null) {
                  throw new hyperbeeError(`missing choices[${index}].tool_calls[${i}].id\n${str(snapshot)}`);
                }
                if (type == null) {
                  throw new hyperbeeError(`missing choices[${index}].tool_calls[${i}].type\n${str(snapshot)}`);
                }
                if (name == null) {
                  throw new hyperbeeError(
