...
export class EventStream<EventTypes extends BaseEvents> {
  ...
  #rejectConnectedPromise: (error: hyperbeeError) => void = () => {};

  #endPromise: Promise<void>;
  #resolveEndPromise: () => void = () => {};
  #rejectEndPromise: (error: hyperbeeError) => void = () => {};
  ...
  #handleError(this: EventStream<EventTypes>, error: unknown) {
    ...
    if (error instanceof hyperbeeError) {
      return this._emit('error', error);
    }
    if (error instanceof Error) {
      const hyperbeeError: hyperbeeError = new hyperbeeError(error.message);
      // @ts-ignore
      hyperbeeError.cause = error;
      return this._emit('error', hyperbeeError);
    }
    return this._emit('error', new hyperbeeError(String(error)));
  }
  ...
  _emit<Event extends keyof EventTypes>(
    ...
    if (event === 'error') {
      ...
      const error = args[0] as hyperbeeError;
      if (!this.#catchingPromiseCreated && !listeners?.length) {
...
export interface BaseEvents {
  connect: () => void;
  error: (error: hyperbeeError) => void;
